...
@RequestMapping("/api/v1/academyByDataFine")
@CrossOrigin("http://localhost:8080")
public class AcademyByDataFineController {

    private AcademyService academyService;

    @GetMapping(path= "/{dataFine}")
    ...
    public ResponseEntity<List<AcademyResponseDTO>> getByDataFine(@PathVariable LocalDate dataFine) {
        try {
            return new ResponseEntity<>(academyService.findByDataFine(dataFine), HttpStatus.OK);
