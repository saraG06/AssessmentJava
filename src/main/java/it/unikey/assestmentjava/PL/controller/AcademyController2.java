...
@RequestMapping("/api/v1/academy_2")
...
public class AcademyController2 {
    ...
    @GetMapping
    @ApiOperation(value = "Method to get a list of Academy that contain a specific name")
    @ApiResponses(value = {
            @ApiResponse(code = 200, message = "Done"),
            @ApiResponse(code = 404, message = "Element not found")
    })
    public ResponseEntity<List<AcademyResponseDTO>> getAcademyByNome(String nome){
        return new ResponseEntity<>(academyService.getAcademyByNome(nome), HttpStatus.OK);
    }
}
